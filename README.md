...
**Order-First 架构**：一次用户操作（开仓/平仓/强平/ADL）= 一个 Order，撮合产生 N 笔 Trade。Order 在调用方创建（`EventCreateOrder`），Trade 在 `UpdatePosition` 中创建（`EventTrade`）。Order ID 由 `book.NextOrderID()` 统一分配，内存和 DB 共享同一套 ID（启动时从 DB MAX 初始化）。
...
**价格监控(Monitor)**：每次外部价格变化时，基于持仓快照检查止盈止损、强平、强盈。止盈止损和强盈走订单簿（市价单），强平走强平引擎（TakeOver）。执行层通过 CAS（TrySetState）保证幂等性。
...
**T+1 对账**：每日自动对账（余额/持仓/零和），不一致时邮件告警。
...
| 持仓引擎(updatePosition) | 统一处理开/加/减仓，双方对称 |
| 做市商机器人 | 跟踪外部价格，自动挂单提供流动性，成交时回调引擎 |
| 标记价格 | 基于现货指数价格 + 合约基差 EMA 平滑，防合约价格操纵 |
