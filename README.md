...
**统一持仓变更**：所有订单（市价/限价/平仓/强平）走同一条路径：订单簿撮合 → `ProcessTrades` → 对买卖双方调 `updatePosition`，自动判断开仓/加仓/减仓/平仓。不区分"开仓API"和"平仓API"在持仓处理层面。

**零和系统**：每笔成交同时处理双方持仓，做市商（UserID=0）有真实账户和持仓追踪。用户盈亏 + 对手方盈亏 + 手续费 = 0，∑多头数量 ≡ ∑空头数量。

**价格监控(Monitor)**：每次外部价格变化时，遍历所有活跃持仓，检查是否触发止盈止损、强平、强盈。触发后通过引擎下反向单平仓，走统一的 `ProcessTrades` 路径。
...
internal/
  engine/
    ...
    trading/       撮合引擎 + 统一持仓变更 (position_update.go)
    clearing/      清算 + 结算
    markprice/     标记价格引擎
    insurance/     保险基金
    adl/           自动减仓
